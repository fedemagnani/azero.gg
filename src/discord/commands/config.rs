...
impl Display for Config {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        map.insert(constants::ACCOUNT_ID_KEY, CommandDataOptionValue::String(self.account_id.clone()));
        map.insert(constants::DECIMALS_KEY, CommandDataOptionValue::Integer(self.decimals as i64));
        map.insert(constants::REQUIRED_AMOUNT_KEY, CommandDataOptionValue::Integer(self.required_amount as i64));
        map.insert(constants::STANDARD_KEY, CommandDataOptionValue::String(self.standard.clone()));
        write!(f, "{:?}", map)
    }
}
...
pub fn register(command: &mut CreateApplicationCommand) -> &mut CreateApplicationCommand {
    ...
    .create_option(|option|{
        option.name(constants::DECIMALS_KEY).description("Decimals").kind(CommandOptionType::Integer).required(true)
    })
    .create_option(|option|{
        option.name(constants::REQUIRED_AMOUNT_KEY).description("Minimum required token amount").kind(CommandOptionType::Integer).required(true)
    })
    .create_option(|option|{
        option.name(constants::STANDARD_KEY)
